...
public class Calculator {
	...
	public Score Calc(HandInfo pHandInfo, IEnumerable<Card> pCryHands, IEnumerable<Card> pHands, IEnumerable<Card> pDoras, IEnumerable<Card> pBackDoras, Card pLastCard, out Form? pForm) {
		
		var haveCried = pCryHands.Any();
		pHandInfo.HaveCried = haveCried;
		
		var score = new Score();
		if (pHandInfo.Nagashi) {
			score.ApplyForm(48, 5, false);
			pForm = null;
			return score;
		}
		score.AddFu(20);
		...
		score.AddFu(form?.GetFu(pHandInfo, pLastCard) ?? 0);
		if (form is not null) {
			score = _normalForms.Aggregate(score,
				(current, checkForm) => checkForm.Calc(current, form, pLastCard, pHandInfo)
			);	
		}
		
		score = _specialForms.Aggregate(score, 
			(current, specialForm) => specialForm.Calc(current, fullHands, pLastCard, pHandInfo, form != null)
		);
		
		CalcDora();

		if (score.Fu == 20) {
			score.ApplyForm(1039, 1);
			if(pHandInfo is { IsRon: true, HaveCried: false }) score.AddFu(10);	
		}
		else {
			if(!pHandInfo.IsRon) score.AddFu(2);
			else if(!pHandInfo.HaveCried) score.AddFu(10);	
		}
		score.CeilFu();
		...
		ExitLoop:
			...
			if(doraCnt > 0)
				score.ApplyForm(1040, doraCnt);
			
			if(pHandInfo.NorthCnt > 0)
				score.ApplyForm(1042, pHandInfo.NorthCnt);
		}
	}
}