...
public record Form(Head pHead, params IEnumerable<Body> pBodies) {
	...
	public int GetFu(HandInfo pHandInfo, Card pLastCard) {
		...
		if (pHandInfo.IsRon) minCollection.IsOpen = true;
		var waitFu = minWaitType is WaitType.NoMiddle or WaitType.SingleFace or WaitType.SingleHead
			? 2 : 0;
		var fu = Bodies.Aggregate(0, (fu, body) => fu + body.GetFu(pHandInfo));
		fu += Head.GetFu(pHandInfo);
		return fu + waitFu;
